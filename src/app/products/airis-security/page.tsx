...
export const metadata = {
  ...
  description: 'AIris Security is an AI-powered vulnerability scanner for web applications. Detect issues early, triage faster, and keep deployments safer.',
};
...
export default function AirisSecurityPage() {
  return (
    <ProductLanding
      ...
      features={[
        { title: 'AI-assisted findings', description: 'Surface likely issues before they become release blockers.', icon: '•' },
        { title: 'Risk-based triage', description: 'Prioritize what matters so teams can act with less noise.', icon: '•' },
        { title: 'Release-aware workflow', description: 'Keep scanning aligned with real delivery cadence.', icon: '•' },
      ]}
      caseStudies={[
        {
          title: 'Release review support',
          description: 'A team used AIris Security to review repeated app deployments before production handoff.',
      ...
      <div style={{ display: 'grid', gap: '14px', width: '100%', fontFamily: 'var(--font-mono)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', border: '1px solid var(--border)', borderRadius: '12px', background: 'rgba(255,255,255,0.03)' }}>
          ...
          <span style={{ fontSize: '10px', color: '#10b981', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#10b981', display: 'inline-block' }} /> Engine Active
          </span>
        </div>
        
        {/* Scanning telemetry logs */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '16px', border: '1px solid var(--border)', borderRadius: '12px', background: 'rgba(0,0,0,0.25)', fontSize: '11px', lineHeight: '1.4' }}>
          <div style={{ color: 'var(--white-dim)' }}>[LOG] Core checks: 48/48 vulnerability modules loaded.</div>
          <div style={{ color: '#10b981' }}>[OK] CORS headers and session cookie configurations.</div>
          <div style={{ color: '#ef4444' }}>[FAIL] SQL injection detected in /api/v1/auth/ledger.ts</div>
          <div style={{ color: '#f59e0b' }}>[WARN] Low entropy secret found in staging profile configs.</div>
          <div style={{ color: '#10b981' }}>[OK] Check complete. 1 Critical flaw, 1 Warning reported.</div>
        </div>

        {/* Mini stats badges */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', textAlign: 'center', fontSize: '10px' }}>
          <div style={{ padding: '8px', border: '1px solid var(--border)', borderRadius: '8px', background: 'rgba(255,255,255,0.02)' }}>
